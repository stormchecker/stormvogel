...
- `model.nr_states` is now a method instead of a property. Use `model.nr_states()` instead of `model.nr_states`.
...
## `bird` module
- When a model type supports actions (like `POMDP`), the `rewards` function provided to `bird.build_bird` must accept two parameters: `(state, action)`. Previously, a single-parameter function might have been incorrectly permitted or used in tests.
